...
const CapsuleStorage = {
  ...
  async initSupabase() {
    ...
    if (res.supabaseUrl && res.supabaseKey) {
      ...
      if (typeof supabase !== 'undefined' && supabase.createClient) {
        this.supabase = supabase.createClient(cleanUrl, res.supabaseKey);
  ...
  async supabaseFetch(method, path, body = null, preferHeader = null) {
    ...
    const url = res.supabaseUrl;
    const key = res.supabaseKey;
  ...
  async saveCapsule(capsule) {
    ...
    if (sb) {
      try {
        ...
        const dbObj = {
          id: uuid,
          user_id: user.id, // Explicitly map authenticated user UUID to comply with RLS
